    ...
    <Card className="overflow-hidden bg-gradient-card border-border">
      <div className="flex items-center gap-3 px-5 py-4 border-b border-border bg-card/50">
        ...
        <div>
          <h3 className={cn('font-display font-extrabold text-xl', lang === 'ar' && 'font-arabic')}>
            {lang === 'ar' ? 'مرشحو الحذاء الذهبي 2026' : 'Golden Boot Favorites 2026'}
          </h3>
          <p className={cn('text-xs text-muted-foreground', lang === 'ar' && 'font-arabic')}>
            {lang === 'ar' ? 'البطولة تنطلق 11 يونيو - لا أهداف بعد' : 'Tournament starts June 11 - no goals scored yet'}
          </p>
        </div>
      ...
      <div className="flex items-center gap-2 mx-4 mt-3 mb-1 text-xs bg-primary/10 border border-primary/20 rounded-lg px-3 py-2">
        ...
        <span className={cn(lang === 'ar' && 'font-arabic')}>
          {lang === 'ar'
            ? 'لم تبدأ البطولة بعد. الأرقام ستتحدث مباشرة من API-Football بعد انطلاق المباريات في 11 يونيو 2026.'
            : 'Tournament has not started. Stats will update live from API-Football once matches begin on June 11, 2026.'}
        </span>
      </div>
            ...
            {!isLoading && scorers.map((scorer: Scorer) => (
              ...
              >
                ...
                <TableCell className="text-center">
                  <span className="inline-flex items-center justify-center min-w-8 h-7 px-2 rounded-md font-display font-extrabold tabular-nums bg-muted text-muted-foreground">
                    -
                  </span>
                </TableCell>
                <TableCell className="text-center hidden sm:table-cell font-semibold tabular-nums text-muted-foreground">-</TableCell>
                <TableCell className="text-center hidden md:table-cell text-muted-foreground tabular-nums">-</TableCell>
              </TableRow>
            ))}
          </TableBody>
        </Table>
