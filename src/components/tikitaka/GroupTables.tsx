    ...
    <div className="space-y-5">
      <div className="flex items-start gap-2 text-xs bg-primary/10 border border-primary/25 rounded-lg px-4 py-3">
        ...
        <p className={cn(lang === 'ar' && 'font-arabic')}>
          {lang === 'ar'
            ? 'الجداول تتحدث من نتائج المباريات النهائية: فوز، تعادل، خسارة، فارق أهداف ونقاط.'
            : 'Tables update from final match results: wins, draws, losses, goal difference and points.'}
        </p>
      </div>
      ...
      <div className="flex items-center gap-3 bg-live/10 border border-live/30 rounded-lg px-4 py-3">
        ...
        <p className={cn('text-sm font-semibold', lang === 'ar' && 'font-arabic')}>
          {lang === 'ar'
            ? 'قبل نهاية أول مباراة ستظهر كل الفرق بأرقام صفرية. بعد كل نتيجة يتم ترتيب المجموعة تلقائياً.'
            : 'Before the first final score, every team starts at zero. Each result updates the group automatically.'}
        </p>
      </div>
